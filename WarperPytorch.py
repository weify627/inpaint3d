...
class Warper:
    ...
    def forward_warp(self, frame1: torch.Tensor, mask1: Optional[torch.Tensor], depth1: torch.Tensor,
                     ...
                     intrinsic2: Optional[torch.Tensor], render_image = True) -> \
        ...
        trans_depth1 = trans_points1[..., 2, 0][:, None] # (b, h, w)
        if self.memory_opt:
            trans_pos = trans_coordinates.permute(0,3,1,2)
            flow12 = None
        else:
            grid = self.create_grid(b, h, w).to(trans_coordinates) # (b, 2, h, w)
        ...
        if self.memory_opt:
            warped_depth2, mask3, idx = self.bilinear_splatting(trans_depth1, mask1, trans_depth1, trans_pos, None,
                                                    is_image=False)#[0] #[:, 0]
        else:
            warped_depth2, mask3, idx = self.bilinear_splatting(trans_depth1, mask1, trans_depth1, flow12, None,
                                                    is_image=False)#[0] #[:, 0]
        return warped_frame2, mask2, warped_depth2, flow12, mask3, idx
    ...
    def compute_transformed_points(self, depth1: torch.Tensor, transformation1: torch.Tensor, transformation2: torch.Tensor,
        ...
        transformation = torch.matmul(transformation2, torch.linalg.inv(transformation1))  # (b, 4, 4)
    ...
    def bilinear_splatting(self, frame1: torch.Tensor, mask1: Optional[torch.Tensor], depth1: torch.Tensor,
        ...
        if self.memory_opt:
            ...
            trans_front = depth1[:, 0] > 0
            id_visible = (trans_in & trans_front).sum(-1).sum(-1)>0
            if id_visible.sum()==0:
                print("all zero")
        ...
        if flow12_mask is None:
            flow12_mask = torch.ones(size=(b, 1, h, w)).to(depth1)
        ...
        # print(prox_weight_se.shape, mask1.shape, flow12_mask.shape, depth_weights.shape)
        # print((prox_weight_se * mask1 * flow12_mask / depth_weights).shape)
        # print(weight_nw.shape)
        ...
        # print(warped_frame.shape, frame1_cl.shape, weight_nw.shape)
        ...
        # warped_frame_cf = torch.moveaxis(warped_frame, [0, 1, 2, 3], [0, 3, 1, 2])
        # warped_weights_cf = torch.moveaxis(warped_weights, [0, 1, 2, 3], [0, 3, 1, 2])
        ...
        if not is_image:
            mask2 = mask2 * (warped_frame2>0)
            warped_frame2 *= mask2

        if is_image:
            # print(warped_frame2.min(),warped_frame2.max(), warped_frame2.shape)
