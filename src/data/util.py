...
def get_ids_uni(label_map):
    ids = list(label_map.values())
    ids_uni = []
    for id in ids:
        if id not in ids_uni:
            ids_uni.append(id)
    ids_uni.sort()  #len=549
    return ids_uni


def get_remapper(ids_uni, ignore_label=1000):
    remapper2 = np.ones(max(ids_uni)+1) * ignore_label
    for i, x in enumerate(ids_uni):
        remapper2[x] = i
    return remapper2
